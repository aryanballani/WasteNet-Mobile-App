...
def process_event_stream(event_stream):
    """
    Process the EventStream object to extract the agent's response.
    
    Args:
        event_stream (EventStream): The event stream object returned by the agent invocation.
    
    Returns:
        str: The full response generated by the agent.
    """
    try:
        messages = []
...
def invoke_agent_with_session(agent_id: str, agent_alias_id: str, query: str, session_id: str = None):
    ...
    try:
        ...
        # Process the event stream and return the output
        event_stream = response.get('completion', None)
        if event_stream:
            agent_output = process_event_stream(event_stream)
            logger.info(f"Agent output: {agent_output}")
            return agent_output
        else:
            logger.warning("No event stream found in the response.")
            return None
